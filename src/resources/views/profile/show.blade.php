@extends('layouts.app')
@section('title', $user->name . ' - X Mock')
@section('content')
...
<div class="border-b border-x-border px-4 py-3 sticky top-0 bg-x-bg/80 backdrop-blur-sm z-10 flex items-center gap-4">
    ...
    <div>
        ...
        <p class="text-x-muted text-sm">{{ $user->posts_count }}件の投稿</p>
    </div>
</div>
...
@if($errors->any())
<script>document.addEventListener('DOMContentLoaded', () => document.getElementById('edit-profile-modal')?.classList.remove('hidden'));</script>
@endif
